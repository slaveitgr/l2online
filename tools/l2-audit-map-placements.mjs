...
const OUT_DIR = join(ROOT, ".l2system-index", "map-placement-audit");
...
const ACTOR_CLASSES = new Set(["StaticMeshActor", "MovableStaticMeshActor", "L2MovableStaticMeshActor"]);
...
function parsePackage(bytes) {
  ...
  const resolveRef = (idx) => {
    ...
    if (idx > 0) return { name: exports[idx - 1]?.objectName ?? "?", pkg: "(this)" };
    return { name: "", pkg: "" };
  };

  return { bytes, dv, names, imports, exports, nm, resolveRef };
}
...
function readProps(pkg, exp) {
  ...
  while (cur.o < end && guard++ < 1000) {
    ...
    if (type === 0x03) props[name] = flag;
    else if (type === 0x04 && size >= 4) props[name] = pkg.dv.getFloat32(valueOffset, true);
    else if (type === 0x05) props[name] = pkg.resolveRef(readCompat32(pkg.bytes, { o: valueOffset }));
    else if (type === 0x0a && structName === "Vector" && size >= 12) {
      props[name] = [
        pkg.dv.getFloat32(valueOffset, true),
        pkg.dv.getFloat32(valueOffset + 4, true),
        pkg.dv.getFloat32(valueOffset + 8, true),
      ];
    } else if (type === 0x0a && structName === "Rotator" && size >= 12) {
      props[name] = [
        pkg.dv.getInt32(valueOffset, true),
        pkg.dv.getInt32(valueOffset + 4, true),
        pkg.dv.getInt32(valueOffset + 8, true),
      ];
    }
...
async function auditMap(mapPath) {
  ...
  for (const exp of pkg.exports) {
    if (!ACTOR_CLASSES.has(exp.className) || exp.size <= 0) continue;
    actorExports++;
    const props = readProps(pkg, exp);
    if (props.bHidden || props.Hidden) hidden++;
    if (props.bDeleteMe || props.DeleteMe) deleted++;
    if (!props.Location || !props.StaticMesh) {
      missingMesh++;
      continue;
    }
    placements.push({ actor: exp.objectName, className: exp.className, mesh: props.StaticMesh.name, pkg: props.StaticMesh.pkg });
  }
  return {
    path: mapPath.replace(ROOT + "/", ""),
    actorExports,
    placements: placements.length,
    hidden,
    deleted,
    missingMesh,
    packages: new Set(placements.map((p) => p.pkg)).size,
    meshes: new Set(placements.map((p) => `${p.pkg}.${p.mesh}`)).size,
    sample: placements.slice(0, 10),
  };
}
...
async function main() {
  ...
  const targets = maps.length ? maps : [
    join(ROOT, "Maps", "17_25.unr"),
    join(ROOT, "Maps", "22_22.unr"),
    join(ROOT, "Maps", "Lobby01.unr"),
  ];
  const results = [];
  for (const target of targets) results.push(await auditMap(target.startsWith("/") ? target : join(ROOT, target)));
