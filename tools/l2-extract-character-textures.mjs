#!/usr/bin/env node
/**
 * l2-extract-character-textures.mjs — adds real diffuse textures to the character
 * models produced by l2-extract-character-meshes.mjs.
 *
 * For each body part `<Race>_mNNN_<code>` it finds the matching diffuse in the body
 * texture package SysTextures/{F,M}<Race>.utx (base skin = `..._mNNN_t00_<code>`,
 * preferring the non-specular/non-normal map), decodes it (DXT1/3/5, RGBA8, P8, G16)
 * to PNG under public/models/tex/<Race>/, and writes the path back into the model JSON.
 *
 * Usage: node tools/l2-extract-character-textures.mjs [Race] [F|M]
 */
...
const SYSTEX = join(ROOT, "SysTextures");
const MODELS = join(ROOT, "public", "models");
const TEXOUT = join(MODELS, "tex");
...
function readPalette(pkg, e) {
  const { b, dv, c32 } = pkg; let o = e.offset;
  const ci = () => { const [v, s] = c32(o); o += s; return v; };
...
function chunk(t, d) { const tt = Buffer.from(t, "latin1"); const l = Buffer.alloc(4); l.writeUInt32BE(d.length, 0); const body = Buffer.concat([tt, d]); const cr = Buffer.alloc(4); cr.writeUInt32BE(crc32(body), 0); return Buffer.concat([l, body, cr]); }
function png(rgba, w, h) { const sig = Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]); const ih = Buffer.alloc(13); ih.writeUInt32BE(w, 0); ih.writeUInt32BE(h, 4); ih[8] = 8; ih[9] = 6; const st = w * 4; const r = Buffer.alloc((st + 1) * h); for (let y = 0; y < h; y++) { r[y * (st + 1)] = 0; Buffer.from(rgba.buffer, rgba.byteOffset + y * st, st).copy(r, y * (st + 1) + 1); } return Buffer.concat([sig, chunk("IHDR", ih), chunk("IDAT", deflateSync(r, { level: 6 })), chunk("IEND", Buffer.alloc(0))]); }

// part name -> diffuse texture export (base skin). The part suffix after `_m0NN_`
// (e.g. "b", "m00_bh", "m00_ah_u00") maps to a texture `…_m0NN_t??[…]<suffix>[_ori]`
// that is NOT a specular (_sp), shadow (_sh) or normal (_n) map. Prefer t00 + _ori.
function matchDiffuse(texExps, partName) {
  const m = partName.match(/_m0(\d\d)_(.+)$/i) || partName.match(/_m0(\d\d)()$/i);
  if (!m) return null;
  const id = m[1];
  const suffix = (m[2] || "").replace(/^m\d+_/i, ""); // drop a leading mesh-variant token
  const code = suffix || "b";
  const notMap = (n) => !/_sp\d?$|_sh$|_n$|_normal$/i.test(n);
  const isTex = (e) => /texture/i.test(e.className);
  const rx = new RegExp(`_m0${id}_t\\d+[a-z]*(?:_m\\d+)?_${code}(?:_ori)?$`, "i");
  let cands = texExps.filter((e) => isTex(e) && rx.test(e.objectName) && notMap(e.objectName));
  if (!cands.length) { // looser: any diffuse for this id whose tail matches the code
    const rx2 = new RegExp(`_m0${id}_t\\d+.*${code}(?:_u\\d+)?(?:_ori)?$`, "i");
    cands = texExps.filter((e) => isTex(e) && rx2.test(e.objectName) && notMap(e.objectName));
  }
  if (!cands.length) return null;
  const score = (n) => (/_ori$/i.test(n) ? 2 : 0) + (/_t00_/i.test(n) ? 1 : 0);
  cands.sort((a, b) => score(b.objectName) - score(a.objectName));
  return cands[0];
}

async function main() {
  const [argRace, argGender] = process.argv.slice(2);
  const files = await readdir(MODELS).catch(() => []);
  const jsons = files.filter((f) => f.endsWith(".json") && (!argRace || f.startsWith(argRace)) && (!argGender || f.includes(`_${argGender}.`)));
  const sysFiles = await readdir(SYSTEX).catch(() => []);
  const pkgCache = new Map(); // utx filename (lowercased) → parsed pkg | null
  const loadTexPkg = async (lcName) => {
    if (pkgCache.has(lcName)) return pkgCache.get(lcName);
    const f = sysFiles.find((x) => x.toLowerCase() === lcName);
    let pkg = null;
    if (f) { try { pkg = parsePkg(decode(new Uint8Array((await readFile(join(SYSTEX, f))).buffer))); } catch { pkg = null; } }
    pkgCache.set(lcName, pkg);
    return pkg;
  };
  for (const jf of jsons) {
    const model = JSON.parse(await readFile(join(MODELS, jf), "utf8"));
    await mkdir(join(TEXOUT, model.race), { recursive: true });
    let ok = 0;
    for (const part of model.parts) {
      // derive the body texture package from the PART prefix (e.g. Ffighter_… → FFighter.utx)
      const pm = part.name.match(/^([FM])([a-z0-9]+?)_m0\d\d/i);
      if (!pm) continue;
      const pkg = await loadTexPkg(`${pm[1]}${pm[2]}.utx`.toLowerCase());
      if (!pkg) continue;
      const te = matchDiffuse(pkg.exps, part.name); if (!te) continue;
      let dec; try { dec = readTexture(pkg, te); } catch { dec = null; } if (!dec) continue;
      const rel = `tex/${model.race}/${te.objectName}.png`;
      await writeFile(join(MODELS, rel), png(dec.rgba, dec.width, dec.height));
      part.texture = "/models/" + rel; ok++;
    }
    await writeFile(join(MODELS, jf), JSON.stringify(model));
    console.log(`  ${jf}: ${ok}/${model.parts.length} parts textured`);
  }
}
main();
